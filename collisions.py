...
class Collisions:
    ...
    def _player_can_move_forward_x(self, position):
        if is_solid(self.get_vid(position + glm.vec3(1, 0, 0))) or is_solid(self.get_vid(position + glm.vec3(1, -PLAYER_HEIGHT + 1, 0))):
    ...
    def _player_can_move_forward_z(self, position):
        if is_solid(self.get_vid(position + glm.vec3(0, 0, 1))) or is_solid(self.get_vid(position + glm.vec3(0, -PLAYER_HEIGHT + 1, 1))):
