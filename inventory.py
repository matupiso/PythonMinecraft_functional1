...
from world_objects.item import StoneHammer
...
class Inventory:
    def __init__(self, app):
        ...
        self.item_names = {SAND:"sand", GRASS:"grass", DIRT:"dirt", STONE:"stone", 
                           ...
                           STONE_HAMMER:"stone_hammer", CHERRIES:"cherries"}
    ...
    def get_crafted_item(self):
        ...
        if craft_group == [WOOD, 0, 0, WOOD]:
            return (STICK, 10), Item(STICK, 10)
        elif craft_group == [COAL, 0, 0, STICK]:
            return (TORCH, 4), Item(TORCH, 4)
        elif craft_group == [STONE, 0, 0, STICK]:
    ...
    def update(self):
        ...
        if self.enabled:
            ...
            if idf and len(idf) == 2:
                ...
                if l and not self.grabbed_item and isinstance(item, Item):
                    self.grabbed_item = item.copy()
                    self.slots[self._idf_to_index(idf)] = 0
                    self.grabbed_item_idf = idf
                elif l and self.grabbed_item_idf and self.grabbed_item_idf != idf and (not item or (item.count + self.grabbed_item.count <= 64)):
                    if not item:

                        self.slots[self._idf_to_index(idf)] = self.grabbed_item.copy()
                        self.grabbed_item = None
                        self.grabbed_item_idf = None
                    elif self.grabbed_item.item_id == item.item_id:
                        self.slots[self._idf_to_index(idf)].count += self.grabbed_item.count
                        self.grabbed_item = None
                        self.grabbed_item_idf = None
                elif r and self.grabbed_item_idf and self.grabbed_item_idf != idf and (not item or (item.count + 1 <= 64)):
                    if not item:
            ...
            elif idf == "e" and l and not self.grabbed_item and not self.grabbed_item_idf and self.get_crafted_item():
