...
class Chunk:
    def __init__(self, world, position):
        ...
        self.is_empty = True
        self.is_full = False
        self.block_data:dict[tuple[int, int, int], BlockData] = {}


        self.center = (glm.vec3(self.position) + 0.5) * CHUNK_SIZE
    

    def get_data_obj(self, pos):
        x,y,z = pos
        if x >= 32 or x < 0 or y >= 32 or y < 0 or z >= 32 or z < 0:
            return 
        if self.block_data.get((x, y, z)):
            self.block_data[(x, y,z)] = BlockData({})
